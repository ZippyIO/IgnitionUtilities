...
public class TilePulverizier extends TileEntity implements IPipeConnection, IPowerReceptor, IInventory, ISidedInventory  {
	
	private PowerHandler powerHandler;
	public double energyCost = 0.1;
	private ItemStack[] inventory = new ItemStack[6];
    public int furnaceTimer;
    public int furnaceCookTime = 20;
	...
	public TilePulverizier() {
		powerHandler = new PowerHandler(this, Type.MACHINE);
		initPowerProvider();
	}

	private void initPowerProvider() {
		powerHandler.configure(10, 10, 25, maxEnergy);
    ...
    public Packet getDescriptionPacket() {
        return IGMod.snw.getPacketFrom(new PulverizierTEMessage(this));
    }
	...
	public void accessBattery() {
		if (this.inventory[slotBattery] != null) {
			...
			if (tag == null) {
				...
				tag.setDouble("MaxInput", 10);
				tag.setDouble("MaxOutput", 10);
			...
			if ((double)energyLevel / (double)maxEnergy < 0.05 && batteryLevel > 10) {
				tag.setInteger("EnergyLevel", batteryLevel - 10);
				powerHandler.setEnergy(energyLevel + 10);
	...
	public void checkUpgrades() {
		...
		if (!worldObj.isRemote) {
			...
			if (upgradeCount > 0) {
				...
				furnaceCookTime = (int) (20 / Math.pow(2,upgradeCount));
			} else  this.furnaceCookTime = 20;
		}
	}
